...
*Setup instructions for a Gitian build of Alchimia Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Alchimia
...
Clone the git repositories for alchimia and Gitian.
...
in the alchimia repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Alchimia Core
----------------

To build Alchimia Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the alchimia repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alchimia/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/magnumopusnetwork/Alchimia
...
For example:
```bash
URL=https://github.com/thrasher-/alchimia.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit alchimia=${COMMIT} --url alchimia=${URL} ../alchimia/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alchimia=${COMMIT} --url alchimia=${URL} ../alchimia/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alchimia=${COMMIT} --url alchimia=${URL} ../alchimia/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the alchimia git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../alchimia/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/alchimia
SIGPATH=/some/root/path/alchimia-detached-sigs

./bin/gbuild --url alchimia=${BTCPATH},signature=${SIGPATH} ../alchimia/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alchimia-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alchimia-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/alchimia-osx-build.assert
```
...
[magnumopusnetwork/gitian.sigs.ltc](https://github.com/magnumopusnetwork/gitian.sigs.ltc/) repository, or if that's not possible create a pull
